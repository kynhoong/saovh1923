...
Bracket.Elements = {
	Screen = function()
		...
		ScreenAsset.Name = "3yy " .. game:GetService("HttpService"):GenerateGUID(false)
	...
	Slider = function(Parent, Window, Slider)
		...
		local function AttachToMouse(Input)
			...
			local ValueNeedConvert = Bracket.Utilities.Scale(ScaleX, 0, 1, Slider.Min, Slider.Max)
			Slider.Value = math.floor(ValueNeedConvert * 10^Slider.Precise) / 10^Slider.Precise
		end
	...
	Dropdown = function(Parent, Window, Dropdown)
		...
		function Dropdown.RemovePressedOption(Self)
			local buttonsToRemove = {}  -- Danh sách tạm thời để lưu trữ các TextButton cần xóa

			-- Tìm và thêm các TextButton cần xóa vào danh sách tạm thời
			for _, Option in pairs(Self.List) do
				if Option.Value then
					table.insert(buttonsToRemove, Option.Object)
				end
			end

			-- Xóa các TextButton từ danh sách tạm thời
			for _, buttonToRemove in ipairs(buttonsToRemove) do
				buttonToRemove:Destroy()
			end

			-- Lọc bỏ các TextButton đã bị xóa khỏi danh sách Self.List
			local newButtonList = {}
			for Index, Option in pairs(Self.List) do
				if not Option.Value then
					table.insert(newButtonList, Option)
					Option.Object.LayoutOrder = #newButtonList
				end
				Option.Object.LayoutOrder = Index
			end

			-- Gán danh sách mới không chứa các TextButton đã bị xóa
			Self.List = newButtonList
			RefreshSelected()
		end

		function Dropdown.RefreshToPlayers(Self, ToggleMode, inputCallback)

			local ServersPlayers = game:GetService("Players")
			local existingPlrlist = {}
			
			if not existingPlrlist[LocalPlayer.Name] then
				existingPlrlist[LocalPlayer.Name] = true
			end

			local function plrlist(inputplr, delete)
				if delete == true then
					Self.RemoveOption(Self, inputplr)
					RefreshSelected()
					print('list deleted', inputplr)
				else
					Self.AddOption(Self, {Name = inputplr.Name, Mode = ToggleMode, Callback = inputCallback})
					print('list added', inputplr)
				end
			end

			local function updatePlayers()
				for _, plr in pairs(PlayerService:GetPlayers()) do
					if not existingPlrlist[plr.Name] then
						plrlist(plr, false)
						existingPlrlist[plr.Name] = true
						print("Thêm người chơi mới:", plr.Name)
					end
				end

				for playerName, _ in pairs(existingPlrlist) do
					if not ServersPlayers:FindFirstChild(playerName) then
						plrlist(playerName, true)
						existingPlrlist[playerName] = nil
						print("Xóa người chơi:", playerName)
					end
				end
			end
			...
			ServersPlayers.PlayerRemoving:Connect(updatePlayers)
		end
